...
```
4. Open Rocket.Chat instance ( http://127.0.0.1:3000 ) and Go through the Rocket.Chat initial setup. Remember
user-name and password for future use (we will also need it in step 9).
...
6. From Rocket Chat open Administration -> General -> Apps and make sure the following options are enabled:
 - Enable App development mode 
 - Enable the App Framework
7. Switch to Command Line and execute the following commands. 
```sh
...
In our case:<br>
8. Administration -> Apps -> Content Moderation.<br>
...
9. Edit [docker-compose-server.yml](https://github.com/shreyanshtomar/moderation/blob/shreyansh_dev/docker-compose-server.yml) in your local moderation folder
...
```sh
 10. cd .. # Make sure you're in moderation directory
